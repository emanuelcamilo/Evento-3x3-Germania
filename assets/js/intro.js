...
function updateScrollEffects() {
  ...
  const heroProgress = Math.min(window.scrollY / (window.innerHeight * .55), 1);
  const root = document.documentElement.style;

  root.setProperty('--background-shift', `${28 + pageProgress * 38}%`);
  root.setProperty('--hero-scale', (1 - heroProgress * .1).toFixed(3));
  root.setProperty('--hero-radius', `${Math.round(heroProgress * 40)}px`);
